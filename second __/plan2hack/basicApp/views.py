...
#     model = Repositories
@api_view(['GET'])
def RepoDetailView(request, pk):
    repo = Repositories.objects.filter(pk=pk)
    print(111111111111111, repo.domain)
    item = repo
    print(22222222222222222, item)
    AllCommits = Commit.objects.filter(commitedRepo=repo.title)
    context = {
        'commits':AllCommits
    }
